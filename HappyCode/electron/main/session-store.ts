...
export class SessionStore {
  ...
  loadSessionMessages(encodedPath: string, sessionId: string): { messages: Array<{ role: 'user' | 'assistant'; text: string; isToolCall?: boolean; toolName?: string }> } {
    ...
    const messages: Array<{ role: 'user' | 'assistant'; text: string; isToolCall?: boolean; toolName?: string }> = []
        ...
        if (entryType === 'user') {
          ...
          for (const block of content as Record<string, unknown>[]) {
            if (block['type'] === 'text' && typeof block['text'] === 'string' && (block['text'] as string).trim()) {
              ...
              messages.push({ role: 'assistant', text: summary, isToolCall: true, toolName: toolName ?? 'Tool' })
            }
          }
        }
      } catch {

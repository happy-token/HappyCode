...
在 session 运行过程中，实时显示当前 session 启动的 shell 进程数量、子 agent 状态、以及关键资源占用。

**需求来源：** 多 agent / hooks 场景下用户不知道后台起了多少进程，容易资源泄漏
**现状：** HappyCode 已有 Subagent DAG 可视化（Phase 2），但缺少 shell 进程级别的监控
**架构思路：**
- 主进程通过 `child_process` 句柄跟踪每个 session 的 shell 进程
- 定期采集：进程数、PID、CPU%、内存 RSS、运行时长
- Agent SDK `canUseTool` 中检测 `Exec` 调用时记录 shell 生命周期
- shell 结束时更新状态，避免僵尸进程残留
- UI：Chat header 右侧小徽章（如 `🖥 3 shells`），点击弹出面板显示详情列表
- 支持手动 Kill 单个 shell 进程
- 与 ROADMAP 的 Tray 功能联动：Tray 显示活跃 shell 数量

**IPC 通道（预留）：**
```typescript
'session:shells'         // { sessionId } → { shells: ShellInfo[] }
'shell:kill'             // { shellPid } → { killed: boolean }
```
...
interface ShellInfo {
  pid: number;
  command: string;       // 启动时的命令
  startedAt: number;     // Unix 秒
  cpuPercent: number;    // 采样值
  memoryRss: number;     // 字节
...
#### 11. Anthropic Compliance API 对接
...
建议优先级：
```
Inline Diff > Session Branching > Session 资源监控 > Token 压力条 > File Touch Map
